...
%         end
        ...
        % This is where the custom processing function is being called.
        function [disp,zone] = applyProcessFunction(self,zone)
            sze = size(zone.y);
            if length(sze)<3
                sze(3) = 1;   % make sure the data is interpreted as a 3D matrix
            end
            ...
            disp = Zone('x',x,'y',cellz,'dy',celldz,'parameter',params);
            
            % link the children and parent objects
            disp = arrayfun(@(x) setfield(x,'parent',zone),disp);
            zone.children = [zone.children disp];
        end
        ...
        function [disp,zone] = makeDisp(self,zone)
...
%             disp = arrayfun(@(x,y) setfield(x,'parent',y),disp,zone);
            % generate the data to populate the zone object
            [disp,zone] = arrayfun(@self.applyProcessFunction,zone,'Uniform',0);
