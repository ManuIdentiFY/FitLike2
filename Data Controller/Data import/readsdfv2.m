...
posParamSummary = find(cellfun(@(x) ~isempty(x),strfind(txt,'PARAMETER SUMMARY')));  % cannot use startWith for compatibility reasons with version older than 2017a
...
posData = find(cellfun(@(x) ~isempty(x),strfind(txt,'DATA')));
